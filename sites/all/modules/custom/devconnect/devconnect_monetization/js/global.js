...
function validateBalanceToTopUp() {
	...
	jQuery("input[name='top_up_amount']", form).val(jQuery("#top-up-balance-input").val());
	if (jQuery("input[name='currency_id']", form).val().length == 0) {
		jQuery("input[name='currency_id']", form).val(jQuery("#currency_selector option:selected").val());
	}
	form.submit();
}
...
function restrictRegexOnChangeEvent(input, regex, hiddenSelector) {
	...
	if (jQuery("#top-up-balance-input").val().length == 0 || regex.test(valueToTest)) {
...
jQuery(function($) {
	...
	$("select option:first-child", "#devconnect-monetization-dowload-prepaid-report-form").attr("selected", true);
	$("select option:first-child", "#devconnect-monetization-dowload-prepaid-report-form").attr("disabled", true);
	...
	$("#donwload_previous_prepaid_statements_anchor").click(function(e) {
		...
		if (message == "") {
			...
			jQuery("#donwload_previous_prepaid_statements_error_div p").html(message);
	...
	$.fn.fixBillingMonthSelect = function() {
		$("#devconnect-monetization-dowload-prepaid-report-form select:last").html($("#devconnect-monetization-dowload-prepaid-report-form select:last div").html());
	};
	...
	//jQuery("#top-up-balance-input").keyup([{input: jQuery("#top-up-balance-input"), regex : /^[1-9][0-9]*((\.[0-9]{1,2})|\.)?$/, hiddenSelector: '#valid_top_up'}], function(e){debugger;});
});
