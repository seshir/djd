...
<div>
  ...
  <h3>Products:&nbsp;<?php echo $product_list; ?></h3>
  <?php if (count($limits)>0): ?>
    ...
    <?php endif; ?>
    <?php foreach ($limits as $product_name => $p_limits): ?>
      <?php echo $product_name . ':<br>'; ?>
      <?php foreach ($p_limits as $product_limit): ?>
        <?php foreach ($rate_plans as $rate_plan2): ?>
          <?php if (count($rate_plan2->getRatePlanDetails()) > 0): ?>
            <?php foreach ($rate_plan2->getRatePlanDetails() as $rate_plan_detail2): ?>
              <?php echo _devconnect_monetization_build_limit_text($product_limit, $rate_plan_detail2->revenueType) . ($product_limit->get_quota_period_type() == 'ROLLING' ? '&nbps;(rolling month)' : ''); break 2; ?>
            <?php endforeach; ?>
          <?php else: ?>
            <?php echo _devconnect_monetization_build_limit_text($product_limit, NULL) . ($product_limit->get_quota_period_type() == 'ROLLING' ? '&nbps;(rolling month)' : ''); break 1; ?>
          <?php endif; ?>
        <?php endforeach;?>
        <br>
      <?php endforeach; ?>
...
<div>
  ...
  <?php foreach ($rate_plan->getRatePlanDetails() as $rate_plan_detail): ?>
    ...
    <?php @list($price_point) = (isset($product)) ? $product->getPricePoints() : NULL; ?>
    <?php if (isset($price_point)): ?>
    ...
    <table>
      ...
      <tbody>
        <tr>
          ...
          <td><?php echo $rate_plan->getCurrency()->name; ?></td>
          <td><?php echo $price_point->getNetStartPrice(); ?></td>
          <td><?php echo $price_point->getNetEndPrice(); ?></td>
        </tr>
      </tbody>
    </table>
    <?php endif; ?>
  ...
  <?php endif; ?>
        ...
        <table>
          ...
          <tbody>
            <tr>
              ...
              <?php if ($rate_plan->getRecurringFee() > 0): ?><td><?php echo _devconnect_monetization_get_frequency_fee_text($rate_plan, $rate_plan_detail); ?></td><?php endif; ?>
        ...
        <div class="purchase-plan well">
          <?php if ($rate_plan->getId() != $active_rate_plan_id): ?>
...
<?php else: ?>
  <?php $rendered_price_points = array(); ?>
  <?php foreach ($rate_plans as $rate_plan): ?>
    <?php foreach ($rate_plan->getRatePlanDetails() as $rate_plan_detail): ?>
      <?php $product = $rate_plan_detail->product; ?>
      <?php list($rate_plan_rates) = $rate_plan_detail->ratePlanRates; ?>
      <?php @list($price_point) = (isset($product)) ? $product->getPricePoints() : NULL; ?>
      <?php if (isset($price_point) && !in_array($product->getId(), $rendered_price_points)): ?>
      <strong>Price Points for <?php echo $product->getDisplayName(); ?></strong>
      <br>
      <br>
      <table>
        ...
        <?php $rendered_price_points[] = $product->getId(); ?>
      <?php endif; ?>
    <?php endforeach; ?>
  <?php endforeach; ?>
