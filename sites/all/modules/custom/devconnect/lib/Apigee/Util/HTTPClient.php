...
class HTTPClient {
  ...
  public static function exec($url, $opts = array()) {
    ...
    if (isset($uri['user']) && isset($uri['pass'])) {
      $curl_options[CURLOPT_HTTPAUTH] = self::parseAuthMethod($opts['auth_method']);
    ...
    switch ($method) {
      ...
      case 'PUT':
        ...
        $fp = fopen('php://temp/maxmemory:256000', 'rw');
        fwrite($fp, $payload);
        rewind($fp);
        $curl_options[CURLOPT_INFILE] = $fp;
        $curl_options[CURLOPT_INFILESIZE] = strlen($payload);
        // keep curl from sending Content-Length header twice; makes the endpoint break
    ...
    foreach ($headers as $header => $header_value) {
      $curl_options[CURLOPT_HTTPHEADER][] = $header . ': ' . $header_value;
    }
    ...
    try {
      ...
      $result->statusMessage = self::getStatusMessage($result->code);
   ...
   * @return int
   */
  private static function parseAuthMethod($auth_type) {
